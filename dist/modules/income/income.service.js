...
function normalizeAmount(income) {
    const obj = income.toObject();
    return {
        ...obj,
        amount: parseFloat(obj.amount.toString()),
        incomeSource: obj.incomeSource && typeof obj.incomeSource === 'object'
            ? {
...
export const IncomeService = {
    ...
    getPaginatedIncomes: (userId, page = 1, limit = 6) => {
        return IncomeRepository.findPaginatedByUser(userId, page, limit).then(({ data, ...meta }) => ({
            ...meta,
            data: data.map(normalizeAmount),
        }));
    },
    getTotalIncomeAmount: async (userId) => {
    ...
    getIncomes: (userId) => IncomeRepository.findAllByUser(userId).then((incomes) => incomes.map(normalizeAmount)),
    updateIncome: (id, userId, updates) => {
        ...
        return IncomeRepository.updateById(id, userId, convertedUpdates).then(normalizeAmount);
    },
    deleteIncome: (id, userId) => IncomeRepository.deleteById(id, userId),
};
